...
def main(bucket, filename, readers=[], owners=[]):
    ...
    resp = upload_object(bucket, filename, filename,readers, owners)
...
def upload_object(bucket, local_filename, gcp_filename, readers, owners):
    ...
    print('Uploading object..')
    req = service.objects().insert(bucket=bucket, body=body, media_body=local_filename)
    resp = req.execute()
